    ...
    GError     **err)
{
    ...
    if (vsp_type != MIO_T_SOCK_DGRAM) {
        ...
        g_set_error(err, MIO_ERROR_DOMAIN, MIO_ERROR_ARGUMENT,
                    "Cannot create UDP sink: type mismatch");
        goto end;
    }
    ...
    if (!hostaddr) {
        ok = FALSE;
        g_set_error(err, MIO_ERROR_DOMAIN, MIO_ERROR_ARGUMENT,
                    "Cannot create UDP sink: no output specifier");
        goto end;
    }

    /* Do lookup and create context */
    if (!(sink->ctx = mio_init_ip_lookup(hostaddr, svcaddr, SOCK_DGRAM,
                                         IPPROTO_UDP, FALSE, err)))
    {
        ok = FALSE;
        goto end;
    }
